...
class Program
{
    public static int[] Filter(int[] source)
    {
        ...
        return RemoveDuplicateElements(source);
    }

    // ДОБАВЬТЕ НОВЫЕ МЕТОДЫ, ЕСЛИ НЕОБХОДИМО

    private static int[] RemoveDuplicateElements(int[] source)
    {
        var list = new List<int>();
        var duplicateElements = new List<int>();
        ...
        for (int i = 0; i < list.Count; i++)
        {
            for (int j = i + 1; j < list.Count; j++)
            {
                if (list[i] == list[j])
                {
                    duplicateElements.Add(list[i]);
                    break;
                }
            }
        }

        list.RemoveAll(x => duplicateElements.Contains(x));
